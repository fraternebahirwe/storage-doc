...
export function FileListRow({
  ...
  return (
    <div className="flex items-center gap-3 border-b border-[var(--color-border)] px-3 py-2.5 last:border-b-0 hover:bg-[var(--color-surface-muted)]">
      <a
        href={fileViewUrl(file.id)}
        target="_blank"
        rel="noreferrer"
        className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-md bg-[var(--color-surface-muted)]"
      >
        {file.category === "image" ? (
      ...
      <span className="hidden w-20 shrink-0 text-xs uppercase text-[var(--color-text-muted)] sm:block">{file.extension}</span>
      <span className="hidden w-20 shrink-0 text-xs text-[var(--color-text-muted)] sm:block">{formatBytes(file.size)}</span>
      <span className="hidden w-28 shrink-0 text-xs text-[var(--color-text-muted)] md:block">{formatDate(file.createdAt)}</span>
      ...
      <div className="relative shrink-0" ref={menuRef}>
        <button
          ...
          className="rounded-md p-1.5 text-[var(--color-text-muted)] hover:bg-[var(--color-surface)] hover:text-[var(--color-text)]"
        >
        ...
        {menuOpen && (
          <div
            role="menu"
            className="absolute right-0 z-10 mt-1 w-40 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] py-1 shadow-lg"
          >
            <a
              role="menuitem"
